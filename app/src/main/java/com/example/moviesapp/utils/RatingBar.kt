...
fun RatingBar(
    ...
    val filledStars = floor(rating).toInt() // округление в меньшую сторону
    val unfilledStars = (stars - ceil(rating)).toInt() // округление в большую сторону
    val halfStars = rating.rem(1) != 0.0 // получаем остаток от деления для проверки нужна ли нам половинка звезды или нет
